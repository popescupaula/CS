...
from enforce_all_audits import enforce_all_audits
from enforce_all_audits import rollback_audits
import webbrowser
import os

PATH = '/home/deep/Desktop/Lab6/'
...
root = Tk()
root.title('Laboratory 6 - Popescu Paula')
...
def open_file():
	...
	if extension == 'audit':
		...
		custom_item['CMD_EXEC'] = {'type' : [], 'description' : [], 'cmd' : [], 'timeout' : [], 'expect' : [], 'dont_echo_cmd': [], 'cmd_to_back_up':[], 'cmd_to_enforce':[]}
...
def profile_data():
	...
	if data == '':
		new_file()
		my_text.insert(END, 'You are not signed in!')
	else:
		new_file()
		my_text.insert(END, 'Name: ')
		my_text.insert(END, data['name'])
		my_text.insert(END, '\n')
		my_text.insert(END, 'Email: ')
		my_text.insert(END, data['email'])
		my_text.insert(END, '\n')
		my_text.insert(END, 'Picture: ')
		my_text.insert(END, data['picture'])
...
def sign_out():
	...
	if data == '':
		new_file()
		my_text.insert(END, 'You have already signed out!')
	else:
		os.remove('profile_data.json')
		new_file()
		my_text.insert(END, 'You signed out!')
...
def sign_in():	
	...
	if data == '':
		...
		new_file()
		my_text.insert(END, 'You have already signed in!')
...
profile_menu.add_command(label = 'Profile Data', command = profile_data)
profile_menu.add_separator()
profile_menu.add_command(label = 'Sign In', command = sign_in)
profile_menu.add_command(label = 'Sign Out', command = sign_out)
